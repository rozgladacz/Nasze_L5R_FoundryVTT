...
export class GmToolboxV2 extends HandlebarsApplicationMixin(ApplicationV2) {
    /** @override ApplicationV2 */
    static get DEFAULT_OPTIONS() { return {
        id: "l5r5e-gm-toolbox-v2",
        window: {
            contentClasses: ["l5r5e", "gm-toolbox-v2"],
        ...
        actions: {
            open_gm_monitor: GmToolboxV2.#openGmMonitor,
            toggle_hide_difficulty: GmToolboxV2.#onToggleHideDifficulty,
            change_difficulty: {
                buttons: [0, 1, 2],
                handler: GmToolboxV2.#onChangeDifficulty
            },
            reset_void: {
                buttons: [0, 1, 2, 3, 4], // all the buttons (left, middle, right, extra 1, extra 2)
                handler: GmToolboxV2.#onResetVoid
            },
            sleep: {
                buttons: [0, 1, 2, 3, 4],
                handler: GmToolboxV2.#onSleep
            },
            scene_end: {
                buttons: [0, 1, 2, 3, 4],
                handler: GmToolboxV2.#onSceneEnd
            },
        }
    }};
    ...
    static PARTS = {
        main: {
            id: "gm-tool-content",
            template: "systems/l5r5e/templates/" + "gm/gm-toolbox-v2.html"
        }
    };
    ...
    static async #onResetVoid(event) {
        ...
        for await (const actor of game.actors.contents) {
            if (!GmToolboxV2.#Helper.updatableCharacter(allActors, actor)) {
                continue;
            }
            await actor.update({
                system: {
        ...
        GmToolboxV2.#Helper.uiNotification(allActors, "reset_void");
    }
    ...
    static async #onSleep(event) {
        ...
        GmToolboxV2.#Helper.uiNotification(allActors, "sleep");
    }
    ...
    static async #onSceneEnd(event) {
        ...
        GmToolboxV2.#Helper.uiNotification(allActors, "scene_end");
    }
