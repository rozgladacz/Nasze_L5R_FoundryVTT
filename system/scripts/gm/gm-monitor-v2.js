...
export class GmMonitorV2 extends HandlebarsApplicationMixin(ApplicationV2) {
    /** @override ApplicationV2 */
    static get DEFAULT_OPTIONS() {
        return {
            id: "l5r5e-gm-monitor-v2",
            tag: "div",
            window: {
                contentClasses: ["l5r5e", "gm-monitor-v2"],
            ...
            actions: {
                add_selected_tokens: GmMonitorV2.#addSelectedTokens,
                change_view_tab: GmMonitorV2.#rotateViewTab,
                remove_actor: GmMonitorV2.#removeActor,
                toggle_prepared: GmMonitorV2.#togglePrepared,
                change_stance: {
                    buttons: [0, 2],
                    handler: GmMonitorV2.#changeStance,
                },
                modify_fatigue: {
                    buttons: [0, 1, 2],
                    handler: GmMonitorV2.#modifyFatigue,
                },
                modify_strife: {
                    buttons: [0, 1, 2],
                    handler: GmMonitorV2.#modifyStrife,
                },
                modify_voidPoint: {
                    buttons: [0, 1, 2],
                    handler: GmMonitorV2.#modifyVoidPoint,
                },
                modify_casualties: {
                    buttons: [0, 1, 2],
                    handler: GmMonitorV2.#modifyCasualties,
                },
                modify_panic: {
                    buttons: [0, 1, 2],
                    handler: GmMonitorV2.#modifyPanic,
                }
            },
    ...
    static async #modifyCasualties(event, target) {
        const {isValid, actor} = await GmMonitorV2.#Helper.getActorValidated(target);
        if(!isValid)
            return;
        ...
        return actor.update({
            system: {
                battle_readiness: {
                    casualties_strength: {
                        value: GmMonitorV2.#Helper.newValue(casualties_strength, event.button),
                    }
                },
            },
        });
    ...
    static async #modifyPanic(event, target) {
        ...
        return actor.update({
            system: {
                battle_readiness: {
                    panic_discipline: {
                        value: GmMonitorV2.#Helper.newValue(panic_discipline, event.button),
                    }
                },
            },
        });
    ...
    static async #modifyFatigue(event, target) {
        ...
        return actor.update({
            system: {
                fatigue: {
                    value: GmMonitorV2.#Helper.newValue(fatigue, event.button)
                }    
            }
        });
    }
    ...
    static async #modifyStrife(event, target) {
        ...
        return actor.update({
            system: {
                strife: {
                    value: GmMonitorV2.#Helper.newValue(strife, event.button),
                },
            },
        });
    }
        ...
        return actor.update({
            system: {
                void_points: {
                    value: Math.min(
                            void_points_max,
                            GmMonitorV2.#Helper.newValue(void_points, event.button)
                    ),
                },
            },
        });
